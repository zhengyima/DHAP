...
class TopKDecoder(torch.nn.Module):
    ...
    def __init__(self, decoder_rnn, k, use_knowledge):
    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None, function=F.log_softmax,
                    teacher_forcing_ratio=0, retain_output_probs=True, r_history_idx=None,
                              r_history_idx_pos=None,
                              r_mask=None,r_mask_pos=None,all_renc_knowledge=None,all_rencs=None,all_pencs=None,all_rencs_byword=None,):
        """
        ...
        if self.rnn.use_attention:
            ...
            # print(all_renc_knowledge)
            if self.use_knowledge:
                all_renc_knowledge = _inflate(all_renc_knowledge, self.k, 0)
                all_rencs = _inflate(all_rencs, self.k, 0)
                all_pencs = _inflate(all_pencs, self.k, 0)
                all_rencs_byword = _inflate(all_rencs_byword, self.k, 0)
                r_history_idx = _inflate(r_history_idx, self.k, 0)
        else:
        ...
        for _ in range(0, max_length):
            ...
            # print("topkdecoder: input_var", input_var.size())
            log_softmax_output, hidden, _ = self.rnn.forward_step(input_var, hidden,
                                                                  inflated_encoder_outputs, function=function, knowledge=all_renc_knowledge, pr_memory=[all_pencs, all_rencs],
